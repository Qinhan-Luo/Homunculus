...
def train(config, model, hooks, optimizer, scheduler, dataloaders, last_epoch):
    ...
    for epoch in range(last_epoch, config.train.num_epochs):
        ...
        if config.scheduler.name == 'CosineAnnealingLR' and epoch % config.scheduler.params.T_max == config.scheduler.params.T_max - 1:
            ...
            adaspace.utils.save_checkpoint(config, model, optimizer, epoch, keep=None,
                                      name=f'snapshot.{snapshot_idx}')
        if ckpt_score > best_ckpt_score:
            best_ckpt_score = ckpt_score
            adaspace.utils.save_checkpoint(config, model, optimizer, epoch, keep=None,
                                      name='best.score')
            adaspace.utils.copy_last_n_checkpoints(config, 5, 'best.score.{:04d}.pth')

        if epoch % config.train.save_checkpoint_epoch == 0:
            adaspace.utils.save_checkpoint(config, model, optimizer,
...
def to_data_parallel(config, model, optimizer):
    if 'sync_bn' in config.train:
        print('sycn bn!!')
        from adaspace.sync_batchnorm import SynchronizedBatchNorm1d, DataParallelWithCallback, convert_model
...
def run(config):
    ...
    model = model.cuda()
    # load checkpoint
    checkpoint = adaspace.utils.get_initial_checkpoint(config)
    if checkpoint is not None:
        last_epoch, step = adaspace.utils.load_checkpoint(model, optimizer, checkpoint)
